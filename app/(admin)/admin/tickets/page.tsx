...
const EMPTY_TIER: TicketTierRow = {
  ...
  description: '',
  privileges: [],
  billing_model: 'per_delegate',
  included_delegates: 1,
...
export default function AdminTicketsPage() {
  ...
  const [tiers, setTiers] = useState<TicketTierRow[]>([]);
  ...
  const handleEdit = (tier: TicketTierRow) => {
    setEditing({ ...tier });
    setIsNew(false);
    setPrivilegeInput('');
  };
  ...
  const handleSave = async () => {
    ...
    if (result.success) {
      toast.success(isNew ? 'Ticket tier created.' : 'Ticket tier updated.');
      setEditing(null);
      setIsNew(false);
      loadTiers();
    } else {
  ...
  const handleAddPrivilege = () => {
    if (!editing || !privilegeInput.trim()) return;
    setEditing({ ...editing, privileges: [...editing.privileges, privilegeInput.trim()] });
    setPrivilegeInput('');
  };

  const handleRemovePrivilege = (index: number) => {
    if (!editing) return;
    const updated = editing.privileges.filter((_, i) => i !== index);
    setEditing({ ...editing, privileges: updated });
  };
        ...
        <div className="bg-nbac-panel border border-nbac-gold/20 rounded-lg p-6 space-y-5 animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="flex items-center justify-between">
            <h3 className="font-display text-lg font-bold text-nbac-gold-light">
              {isNew ? 'Create Ticket Tier' : `Editing: ${editing.name}`}
            </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <input
                type="text"
                placeholder="Executive Delegate"
                value={editing.badge ?? ''}
            ...
            {/* Billing Model */}
            <div className="space-y-1.5">
              <label className="font-sans text-xs uppercase tracking-wider font-semibold text-nbac-muted">Billing Model</label>
              <select
                value={editing.billing_model}
                onChange={(e) => setEditing({ ...editing, billing_model: e.target.value })}
                className="w-full bg-nbac-canvas border border-nbac-border rounded-md px-3 py-2 font-sans text-sm text-nbac-text focus:border-nbac-gold/40 focus:outline-none transition-colors"
              >
                <option value="per_delegate">Per Delegate</option>
            ...
            {/* Included Delegates */}
            <div className="space-y-1.5">
              <label className="font-sans text-xs uppercase tracking-wider font-semibold text-nbac-muted">Included Delegates</label>
              <input
                type="number"
                min={1}
                value={editing.included_delegates}
                onChange={(e) => setEditing({ ...editing, included_delegates: Number(e.target.value) })}
                className="w-full bg-nbac-canvas border border-nbac-border rounded-md px-3 py-2 font-sans text-sm text-nbac-text focus:border-nbac-gold/40 focus:outline-none transition-colors"
              />
            </div>
          ...
          <div className="space-y-2">
            <label className="font-sans text-xs uppercase tracking-wider font-semibold text-nbac-muted flex items-center gap-1.5">
              <ListChecks size={12} /> Privileges ({editing.privileges.length})
            </label>
            <div className="space-y-1.5">
              {editing.privileges.map((priv, i) => (
                <div key={i} className="flex items-center gap-2 bg-nbac-canvas border border-nbac-border rounded-md px-3 py-1.5 group">
                  ...
                  <button
                    onClick={() => handleRemovePrivilege(i)}
            ...
            <div className="flex gap-2">
              <input
                type="text"
                placeholder="Add a privilege..."
                value={privilegeInput}
                onChange={(e) => setPrivilegeInput(e.target.value)}
                onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleAddPrivilege(); } }}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-nbac-border font-sans text-sm">
              {loading ? (
                <tr>
                  <td colSpan={8} className="p-8 text-center text-nbac-muted font-sans text-xs">
                ...
                <tr>
                  <td colSpan={8} className="p-8 text-center text-nbac-muted font-sans text-xs">
                    No ticket tiers found. The public site will fall back to hardcoded defaults.
                    <br />
                ...
                tiers.map((tier, idx) => (
                  <tr key={tier.id} className="hover:bg-nbac-canvas/40 transition-colors">
                    ...
                    <td className="p-4">
                      ...
                      <span className="text-nbac-muted text-xs font-mono">id: {tier.id}</span>
                    </td>
                    <td className="p-4 text-nbac-emerald-light font-semibold">{formatCurrency(tier.price)}</td>
                    <td className="p-4 text-nbac-body capitalize">{tier.billing_model.replace('_', ' ')}</td>
                    <td className="p-4 text-nbac-body text-center">{tier.included_delegates}</td>
                    <td className="p-4">
                    ...
                    <td className="p-4 text-nbac-muted text-xs">{tier.privileges.length} items</td>
